...
**Domain:** AI-powered customer-support email automation (classify → extract → ground → draft → send via Freshdesk), with dual MCP servers (Selless transactional + Knowledge RAG) and an offline evaluation harness. English-only, ~23k emails / 7 days.
**Researched:** 2026-05-27
**Confidence:** HIGH on LLM/MCP/Freshdesk/eval core; MEDIUM on RAG component specifics and future-phase voice/chatbot tooling (verified against current sources, but those phases are not yet designed).

> **Stance:** This is a Python-primary stack. The CS-automation pipeline (classification, RAG, eval, guardrails) lives in the richest LLM ecosystem (Python). The two MCP servers can be either language; recommendation below splits by where each server's data lives.
...
| **PydanticAI** | latest 1.x | Agent/orchestration layer for the email pipeline | Type-safe, schema-enforced outputs (Pydantic models for classification labels + extracted fields) — exactly the structured-output discipline this pipeline needs. Lightweight, model-agnostic, first-class MCP client support, far less ceremony than LangGraph for a mostly-linear pipeline. |
...
uv add anthropic pydantic-ai mcp "psycopg[binary]" pgvector voyageai httpx tenacity

# RAG + eval
uv add ragas deepeval
uv add presidio-analyzer presidio-anonymizer
uv add nemoguardrails        # or: uv add guardrails-ai
uv add langfuse opentelemetry-sdk

# Dev / eval gate
uv add --dev pytest promptfoo   # promptfoo is also available via npx for CLI/CI
...
**If eval reveals retrieval (not generation) is the weak link:**
...
**For the offline eval harness specifically:**
...
**Chatbot / Contact Form (≈phase 3, deferred):**
...
---
*Stack research for: AI customer-support email automation (Freshdesk + dual MCP + RAG + offline eval)*
*Researched: 2026-05-27*
